...
w=2.0;N=100;
%1 产生Ｎ个测试包
%正包样本(同时有(0,0)(5,5)为正),没有或仅有一个的,为负 
for i=1:50
    TrainBag(i).name=['Bag' num2str(i)]; %包名
    TrainBag(i).label=1;   %包号
    %每个包中有五个示例
    Temp=zeros(5,2);
    Temp(1,:)=randn(1,2);
    Temp(2,:)=w+randn(1,2);
    Temp(3,:)=-w+randn(1,2);
    Temp(4,:)=[randn-w randn+w];
    Temp(5,:)=[randn+w randn-w];
    TrainBag(i).instance=Temp;   
    TrainBag(i).inst_label=[1 1 1 1 1];   
    TrainBag(i).inst_name=['INST1' 'INST2' 'INST3' 'INST4' 'INST5'];   
end
%负包样本
for i=51:70
    TrainBag(i).name=['Bag' num2str(i)]; %包名
...
for i=71:80
    ...
    %每个包中有五个示例
    Temp=zeros(5,2);
    Temp(1,:)=[randn-w*0.8 randn+w*0.8]';
    Temp(2,:)=randn(1,2)-w;
    Temp(3,:)=-w+randn(1,2);
    Temp(4,:)=[randn-w randn+w];
    Temp(5,:)=[randn+w randn-w];
...
%2 将训练样本显示出来
figure(1);
for i = 1:length(TrainBag)
    hold on;
    if TrainBag(i).label == 1
        H1 = plot(TrainBag(i).instance(:,1),TrainBag(i).instance(:,2),'m+');
    else
        H2 = plot(TrainBag(i).instance(:,1),TrainBag(i).instance(:,2),'b*');
    end
end
% legend([H1, H2],'instances in positive bags','instances in negative bags',...
%     'Location','Best');
% title('Training set: 50 positive bags + 50 negative bags');
disp('Data is Ready...')

%2 进行视觉语义提取
%2.1 将所有示例取出,组成示例集 求邻域半径
InstSet=[]; 
for n=1:length(TrainBag)
    InstSet=[InstSet;TrainBag(n).instance];
end
%2.2 求示例两两之间距离
N=size(InstSet,1);DisMat=zeros(N,N);
TSum=0;TNum=0;
for n=1:N
    for m=n+1:N
        %DisMat(n,m)=10/sqrt(sum((InstSet(n,:)-InstSet(m,:)).^2));
        D=sum( ( InstSet(n,:)-InstSet(m,:) ).^2) ;
        D1=sqrt(D);
        DisMat(n,m)=-D; %exp(-D/5);
...
for n=1:size(VSS,1)
     H3=plot(VSS(n,1),VSS(n,2),'ro');
end

for n=1:size(VS,1)
    v=VS(n,:);
    H4=plot(v(1,1),v(1,2),'ks',...
               'LineWidth',2,...
                'MarkerEdgeColor','k',...
