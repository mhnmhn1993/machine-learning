...
knoise=6;
%边缘检测
[M, m, or, featType, PC, EO] = phasecong2(im,4 ,6 , f, 2.1, 0.55, 1.2, knoise,0.4 ,10 );%改进的相位一致性检测边缘
M=M+m;
...
figure,imhist(hist);
x=imhist(im);
num=histogsmooth(x,100);%平滑直方图，100为平滑的次数
[mu,mask]=kmeans1(hist,num);%对种子点进行kmeans聚类
